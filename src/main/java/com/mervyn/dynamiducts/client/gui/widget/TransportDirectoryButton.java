...
public class TransportDirectoryButton extends AbstractWidget {
  ...
  public TransportDirectoryButton(
      int x,
      int y,
      int width,
      int height,
      Identifier texture,
      Consumer<TransportDirectoryEntry> onPress) {
  ...
  protected void extractWidgetRenderState(
    ...
    graphics.blit(
        ...
        getY(),
        BASE_U,
        isHoveredOrFocused() ? HOVER_V : BASE_V,
        width,
        height,
        256,
        256);
    ...
    if (!entry.icon().isEmpty()) {
      graphics.item(entry.icon(), getX() + 3, getY() + 3);
    }
    ...
    graphics
        ...
        .accept(
            getX() + height + 4,
            getY() + (height - 8) / 2,
            Component.literal(text).withStyle(style -> style.withColor(getTextColor())));
