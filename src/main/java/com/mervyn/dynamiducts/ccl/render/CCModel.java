...
public class CCModel implements IVertexSource, Copyable<CCModel> {
  ...
  public boolean hasAttribute(AttributeKey<?> attr) {
    return attr.attributeKeyIndex < attributes.size()
        && attributes.get(attr.attributeKeyIndex) != null;
  }
  ...
  /**
   * Ensires the specified {@link AttributeKey}'s index is available in the {@link #attributes}
   * list.
   *
   ...
   * @return The generated model. When rendering an icon will need to be supplied for the UV
   *     transformation.
   */
  ...
  public CCModel generateBlock(
      int i, double x1, double y1, double z1, double x2, double y2, double z2) {
   ...
   * @param y2 maxY
   * @param z2 maxZ
   * @param mask A bitmask of sides NOT to generate. I high bit at index s means side s will not be
   *     generated
   * @return The generated model. When rendering an icon will need to be supplied for the UV
   *     transformation.
   */
  public CCModel generateBlock(
      int i, double x1, double y1, double z1, double x2, double y2, double z2, int mask) {
    double u1, v1, u2, v2;
  ...
  /**
   * Computes the normals of all faces in the model. Uses the cross product of the vectors along 2
   * sides of the face
   *
   * @param start The first vertex to generate normals for
   * @param length The number of vertices to generate normals for. Note this must be a multiple of 3
   *     for triangles or 4 for quads
   * @return The model
   */
  ...
  /**
   * Computes lighting using the normals add a light model If the model is rotated, the lighting
  ...
  /**
   * Brings the UV coordinates of each face closer to the center UV by d. Useful for fixing texture
   * seams
   */
   ...
   *
   * <p>This will deeply copy all vertex data and attributes. {@link ModelMaterial} attributes will
   * only be copied to the destination model if the provided {@code srcpos} and {@code destpos} are
   * {@code 0}.
   *
   ...
   * @return The model
   */
  public static CCModel generateBackface(
      CCModel src, int srcpos, CCModel dst, int destpos, int length) {
    int vp = src.vp;
  ...
  /**
   * Generates sided copies of vertices into this model. Assumes that your model has been generated
   * at vertex side*(numVerts/6)
   */
  ...
  public CCModel generateSidedPart(
      int side1, int side2, Vector3 point, int srcpos, int destpos, int length) {
    return apply(
        new TransformationList(sideRotations[side1].inverse(), sideRotations[side2]).at(point),
        srcpos,
        destpos,
   ...
   *
   * <p>This will deeply copy all vertices and attributes. The returned model will have the {@link
   * ModelMaterial} of the first provided Model.
   *
  ...
  public static CCModel combine(Collection<CCModel> models) {
    if (models.isEmpty())
      throw new IllegalArgumentException("Can't combine an empty collection of models.");
