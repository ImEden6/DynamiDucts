...
public enum EnumColour implements StringRepresentable {
  ...
  ORANGE(
      "orange", "c:dyes/orange", "c:wools/orange", "item.minecraft.firework_star.orange", 0xC06300),
  MAGENTA(
      "magenta",
      "c:dyes/magenta",
      "c:wools/magenta",
  ...
  LIGHT_BLUE(
      ...
      0x6F84F1),
  YELLOW(
      "yellow", "c:dyes/yellow", "c:wools/yellow", "item.minecraft.firework_star.yellow", 0xBFBF00),
  ...
  PURPLE(
      "purple", "c:dyes/purple", "c:wools/purple", "item.minecraft.firework_star.purple", 0x5E00C0),
  ...
  public static final StringRepresentable.EnumCodec<EnumColour> CODEC =
      StringRepresentable.fromEnum(EnumColour::values);
  public static final IntFunction<EnumColour> BY_ID =
      ByIdMap.continuous(EnumColour::ordinal, values(), ByIdMap.OutOfBoundsStrategy.WRAP);
  public static final StreamCodec<ByteBuf, EnumColour> STREAM_CODEC =
      ByteBufCodecs.idMapper(BY_ID, EnumColour::ordinal);
  ...
  private static final Map<String, EnumColour> nameLookup =
      FastStream.of(values()).toMap(e -> e.name, Function.identity());

  private static final Map<Identifier, EnumColour> dyeTagLookup =
      FastStream.of(values()).toMap(e -> e.dyeTagName, Function.identity());

  private static final Map<Identifier, EnumColour> woolTagLookup =
      FastStream.of(values()).toMap(e -> e.woolTagName, Function.identity());
  ...
  public static @Nullable EnumColour fromDyeStack(ItemStack stack) {
    return FastStream.of(stack.typeHolder().tags().toList())
        .map(TagKey::location)
