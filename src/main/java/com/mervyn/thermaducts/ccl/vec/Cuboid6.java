...
public class Cuboid6 implements Copyable<Cuboid6> {
  ...
  public Cuboid6(CompoundTag tag) {
    this(
        Vector3.fromNBT(tag.getCompound("min").orElse(new CompoundTag())),
        Vector3.fromNBT(tag.getCompound("max").orElse(new CompoundTag())));
  }
  ...
  public Cuboid6 expandSide(Direction side, int amount) {
    switch (side.getAxisDirection()) {
      case NEGATIVE ->
          min.add(new Vector3(side.getStepX(), side.getStepY(), side.getStepZ()).multiply(amount));
      case POSITIVE ->
          max.add(new Vector3(side.getStepX(), side.getStepY(), side.getStepZ()).multiply(amount));
    }
    return this;
  }
