...
/**
 * Note that when using the class as a vertex transformer, the vertices are assumed to be within the
 * BB (x, y, z) -> (x+1, y+1, z+1)
 */
public class LightMatrix implements IVertexOperation {
  ...
  /**
   * The 9 positions in the sample array for each side, sides >= 6 are centered on sample 13 (the
   * block itself)
   */
  // @formatter:off
  ...
  public static final int[][] qsamplem =
      new int[][] { // the positions in the side sample array for each corner
        {0, 1, 3, 4},
        {5, 1, 2, 4},
        {6, 7, 3, 4},
        {5, 7, 8, 4}
  ...
  public void sample(int i) {
    ...
    if ((sampled & 1 << i) == 0) {
      BlockPos bp =
          new BlockPos(
              pos.getX() + (i % 3) - 1, pos.getY() + (i / 9) - 1, pos.getZ() + (i / 3 % 3) - 1);
