...
public interface IVertexOperation {
  ...
  /**
   * Load any required references and add dependencies to the pipeline based on the current model
   * (may be null) Return false if this operation is redundant in the pipeline with the given model
   */
  ...
  /**
   * Get the unique id representing this type of operation. Duplicate operation IDs within the
   * pipeline may have unexpected results. ID should be obtained from
   * CCRenderState.registerOperation() and stored in a static variable
   */
  int operationID();
