...
import com.mervyn.dynamiducts.ccl.model.CachedFormat;
import com.mervyn.dynamiducts.ccl.render.buffer.ISpriteAwareVertexConsumer;
...
import com.mervyn.dynamiducts.ccl.render.lighting.LightMatrix;
import com.mervyn.dynamiducts.ccl.render.lighting.PlanarLightModel;
import com.mervyn.dynamiducts.ccl.render.pipeline.CCRenderPipeline;
...
/**
 * The core of the CodeChickenLib render system. Where possible assign a local var of CCRenderState
 * to avoid millions of calls to instance(); Uses a ThreadLocal system to assign each thread their
 * own CCRenderState so we can use it in Multithreaded chunk batching.
 *
 * <p>Piping Vanilla {@link BakedQuad} instances through here possible via {@link
 * BakedVertexSource}.
 */
public class CCRenderState {

  private static final ThreadLocal<CCRenderState> instances =
      ThreadLocal.withInitial(CCRenderState::new);
  ...
  // context
  /**
   * The base color, multiplied by the {@link ColourAttribute} from the bound model if present
  ...
  /**
   * Lets the {@link LightMatrix} or {@link PlanarLightModel} know if this {@link CCRenderState}
   * should compute lighting.
   */
  ...
  /**
   * Bind this {@link CCRenderState} to the given {@link RenderType}, applying the given
   * MatrixStack.
   *
   * @param renderType The {@link RenderType} to bind to.
   * @param source The {@link MultiBufferSource} instance.
  ...
  public void render() {
    ...
    if (fmt == null)
      throw new IllegalStateException(
          "VertexFormat is not bound?"); // this should be handled by the VertexConsumer assertion.
  ...
  public void writeVert() {
    ...
    for (VertexFormatElement fmte : elements) {
      ...
      } else if (fmte == VertexFormatElement.UV
          || fmte == VertexFormatElement.UV0
          || fmte == VertexFormatElement.UV1
          || fmte == VertexFormatElement.UV2) {
        switch (fmte.index()) {
  ...
  public void setBrightness(Entity entity, float frameDelta) {
    brightness =
        Minecraft.getInstance()
            ...
            .getPackedLightCoords(entity, frameDelta);
  }
  ...
  public void setFluidColour(FluidStack fluidStack, int alpha) {
    int tint =
        Minecraft.getInstance()
            ...
            .fluidTintSource()
            .colorAsStack(fluidStack);
    this.baseColour = (tint & 0xFFFFFF) << 8 | alpha;
  }
