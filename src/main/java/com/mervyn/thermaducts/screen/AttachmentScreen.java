...
public class AttachmentScreen extends AbstractContainerScreen<AttachmentMenu> {
  ...
  public InfoSideTabWidget getInfoTab() {
    return infoTab;
  }

  public RedstoneControlTabWidget getRedstoneTab() {
    return redstoneTab;
  }
  ...
  protected void init() {
    ...
    redstoneTab =
        addRenderableWidget(
            new RedstoneControlTabWidget(
                tabX,
                topPos + 32,
                () ->
                    RedstoneMode.values()[
                        Math.min(menu.getRedstoneMode(), RedstoneMode.values().length - 1)],
                this::setRedstoneMode));
    infoTab = addRenderableWidget(new InfoSideTabWidget(tabX, topPos + 4, this::getInfoTabLines));
  }
  ...
  public void extractBackground(
    ...
    for (int row = 0; row < menu.gridHeight; row++) {
      for (int col = 0; col < menu.gridWidth; col++) {
        ...
        graphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x, y, 7f, 122f, 18, 18, 256, 256);
      }
    }
  }
  ...
  protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
    ...
    if (showsRetainControls(attachment)) {
      ...
      if (retainSize == 0) {
        ...
        graphics.text(font, Integer.toString(retainSize), xQty, 18, 0x404040, false);
      }
    }
    ...
    if (showsStackControls(attachment)) {
      ...
      graphics.text(font, text, xQty, 46, 0x404040, false);
    }
  }
