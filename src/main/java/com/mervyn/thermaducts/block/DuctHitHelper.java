...
public final class DuctHitHelper {
  ...
  private static final AABB CENTER_SUPER =
      new AABB(0.26875, 0.26875, 0.26875, 0.73125, 0.73125, 0.73125);
  private static final AABB CENTER_LARGE = new AABB(0.125, 0.125, 0.125, 0.875, 0.875, 0.875);
  private static final AABB CENTER_TRANSPORT =
      new AABB(0.0625, 0.0625, 0.0625, 0.9375, 0.9375, 0.9375);
  ...
  public static DuctHit resolve(
      BlockState state, @Nullable DuctBlockEntity ductBE, BlockPos pos, BlockHitResult hitResult) {
    ...
    for (Direction dir : Direction.values()) {
      if (hasInteractiveAttachment(ductBE, dir)
          && attachmentBox(dir, collars, cables).inflate(0.01).contains(hit)) {
  ...
  private static Direction pickConnectedSide(
      Vec3 hit, BlockState state, AABB[] collars, AABB[] cables, Direction fallbackSide) {
    Direction best = null;
  ...
  private static Direction pickClosestSide(
      Vec3 hit, AABB[] collars, AABB[] cables, Direction fallbackSide) {
    Direction best = null;
    double bestDist = Double.POSITIVE_INFINITY;
    for (Direction dir : Direction.values()) {
      AABB box = unionBox(collars[dir.ordinal()], cables[dir.ordinal()]);
  ...
  public static AABB outlineBox(
      DuctHit hit, @Nullable DuctBlockEntity ductBE, @Nullable BlockState state) {
    ...
    return switch (hit.part()) {
      case COLLAR ->
          hasInteractiveAttachment(ductBE, hit.side())
              ? attachmentBox(hit.side(), collars, cables)
              : collars[hit.side().ordinal()];
  ...
  private static void genBoxes(AABB[] boxes, double min, double innerMin, double innerMax) {
    ...
    boxes[Direction.UP.ordinal()] =
        new AABB(innerMin, 1.0 - min, innerMin, innerMax, 1.0, innerMax);
    boxes[Direction.NORTH.ordinal()] = new AABB(innerMin, innerMin, 0.0, innerMax, innerMax, min);
    boxes[Direction.SOUTH.ordinal()] =
        new AABB(innerMin, innerMin, 1.0 - min, innerMax, innerMax, 1.0);
    boxes[Direction.WEST.ordinal()] = new AABB(0.0, innerMin, innerMin, min, innerMax, innerMax);
    boxes[Direction.EAST.ordinal()] =
        new AABB(1.0 - min, innerMin, innerMin, 1.0, innerMax, innerMax);
  }

  private static boolean hasInteractiveAttachment(
      @Nullable DuctBlockEntity ductBE, Direction side) {
