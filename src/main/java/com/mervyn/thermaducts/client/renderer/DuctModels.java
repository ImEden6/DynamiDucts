...
public class DuctModels {
  ...
  private static void generateConnections() {
    modelConnection[0][1] =
        CCModel.quadModel(48)
            .generateBlock(
                0, new Cuboid6(0.3125, 0.6875, 0.3125, 0.6875, 1, 0.6875).expand(-RENDER_OFFSET));
    modelConnection[1][1] =
        CCModel.quadModel(24)
            .generateBox(0, -4, 4, -4, 8, 4 - RENDER_OFFSET, 8, 0, 0, 32, 32, 16)
            .computeNormals();
    modelConnection[2][1] =
        CCModel.quadModel(24)
            .generateBox(0, -4, 4, -4, 8, 4 - RENDER_OFFSET, 8, 0, 16, 32, 32, 16)
            .computeNormals();
  ...
  private static void generateFrames() {
    modelFrameConnection =
        new DuctModelHelper.OctagonalTubeGen(0.375, 0.1812, true).generateModels();
    modelFrame =
        new DuctModelHelper.OctagonalTubeGen(0.375 * LARGE_INNER_SCALE, 0.1812, false)
            .generateModels();
    modelTransportConnection =
        new DuctModelHelper.OctagonalTubeGen(0.5 * LARGE_INNER_SCALE, 0.1812, true)
            .generateModels();
    modelTransport =
        new DuctModelHelper.OctagonalTubeGen(
                0.5 * LARGE_INNER_SCALE * LARGE_INNER_SCALE, 0.1812, false)
            .generateModels();
    modelSideTubes = new DuctModelHelper.SideTubeGen(W + 1.0E-4).generateModels();
    modelSideTubesInner =
        new DuctModelHelper.SideTubeGen(W + 1.0E-4).contract(0.999).generateModels();
  }
  ...
  private static void addSideFace(
      LinkedList<Vertex5> verts, BlockRenderer.BlockFace face, Cuboid6 bounds, int side) {
