...
final class DuctModelHelper {
  ...
  private static final int[][] ORTHOG_AXES = {{2, 4}, {2, 4}, {0, 4}, {0, 4}, {0, 2}, {0, 2}};
  ...
  static final class OctagonalTubeGen {
    ...
    private CCModel generateConnection() {
      ...
      for (int i = 0; i < 8; i++) {
        model.verts[i * 4] =
            new Vertex5(octoFace[i].copy().multiply(inflate, 1, inflate), 0.5 - innerSize, 0);
        model.verts[i * 4 + 1] =
            new Vertex5(
                octoFace[i].copy().multiply(inflate, 1, inflate).setSide(0, -inner),
                0.5 - innerSize,
                0.5 - inner);
        model.verts[i * 4 + 2] =
            new Vertex5(
                octoFace[(i + 1) % 8].copy().multiply(inflate, 1, inflate).setSide(0, -inner),
                0.5 + innerSize,
                0.5 - inner);
        model.verts[i * 4 + 3] =
            new Vertex5(
                octoFace[(i + 1) % 8].copy().multiply(inflate, 1, inflate), 0.5 + innerSize, 0);
      }
    ...
    private LinkedList<Vertex5> generateIntersections(int connections) {
      ...
      LinkedList<Vertex5> center =
          addSideFace(
              new LinkedList<>(),
              new Cuboid6(-innerSize, -size, -innerSize, innerSize, size, innerSize),
              0);
          ...
          if (!a && !b) {
            vertices.add(
                toVertex5(
                    v1.copy()
                        .multiply(size)
                        .add(v2.copy().multiply(innerSize))
                        .add(v3.copy().multiply(innerSize)),
                    i));
            vertices.add(
                toVertex5(
                    v1.copy()
            ...
            if (count == 3) {
              ...
              vertices.add(
                  toVertex5(
                      a1.copy()
                          .multiply(innerSize)
                          .add(a2.copy().multiply(size))
                          .add(a3.copy().multiply(innerSize)),
                      0));
              vertices.add(
                  toVertex5(
                      a1.copy()
              ...
              vertices.add(
                  toVertex5(
                      a1.copy()
                          .multiply(size)
                          .add(a2.copy().multiply(innerSize))
                          .add(a3.copy().multiply(size)),
                      0));
              vertices.add(
                  toVertex5(
                      a1.copy()
  ...
  static final class SideTubeGen {
        ...
        for (int y = -1; y <= 1; y += 2) {
          Cuboid6 cube =
              newCube(
                  ...
                  a.copy()
                      .multiply(h / 2)
                      .add(b.copy().multiply(s2 * x))
                      .add(c.copy().multiply(s2 * y)));
    ...
    private LinkedList<Vertex5> generateIntersections(int connections) {
          ...
          for (int y = -1; y <= 1; y += 2) {
            Cuboid6 cube =
                newCube(
                    a.copy()
                        .multiply(s2)
                        .add(b.copy().multiply(s * x))
                        .add(c.copy().multiply(s * y)),
                    a.copy()
                        .multiply(h / 2)
                        .add(b.copy().multiply(s2 * x))
                        .add(c.copy().multiply(s2 * y)));
      ...
      for (int[] corner : CORNER_TRIPLETS) {
        ...
        int mask =
            ((1 << corner[0]) & connections)
                | ((1 << corner[1]) & connections)
                | ((1 << corner[2]) & connections);
