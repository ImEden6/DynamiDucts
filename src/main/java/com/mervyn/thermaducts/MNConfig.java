...
public class MNConfig {
  ...
  private static final ModConfigSpec.IntValue ENERGY_BASE_TRANSFER =
      BUILDER
          .comment(
              "Base energy transfer rate (RF/t) for Leadstone Fluxduct. Higher tiers scale from this.")
          .defineInRange("energy.baseTransfer", 1000, 100, 10000);

  private static final ModConfigSpec.IntValue FLUID_BASE_CAPACITY =
      BUILDER
          .comment("Base fluid capacity (mB) per duct in a fluid network.")
          .defineInRange("fluid.baseCapacity", 3000, 500, 24000);

  private static final ModConfigSpec.IntValue FLUID_BASE_THROUGHPUT =
      BUILDER
          .comment(
              "Base fluid throughput (mB/t) for a fluiduct grid. Matches Thermal Dynamics 1.12 water flow.")
          .defineInRange("fluid.baseThroughput", 120, 20, 6000);

  private static final ModConfigSpec.IntValue ITEM_BASE_SPEED =
      BUILDER
          .comment("Base item travel speed in ticks per duct.")
          .defineInRange("item.baseSpeed", 40, 10, 200);

  private static final ModConfigSpec.IntValue ITEM_FAST_SPEED =
      BUILDER
