...
public class FluidDuctUnitTemperate extends FluidDuctUnit {
  ...
  public void tickTemperature() {
    ...
    if (internalTemperature < FREEZING_TEMPERATURE || internalTemperature > MELTING_TEMPERATURE) {
      if (serverLevel.getRandom().nextInt(BREAK_CHANCE) == 0) {
    ...
    if (level.getRandom().nextInt(6) == 0) {
      ...
      if (fluidBlock instanceof LiquidBlock && level.getBlockState(pos).isAir()) {
        if (fluid.getFluid() == Fluids.WATER
          ...
          level.setBlock(
              pos, fluid.getFluid().defaultFluidState().createLegacyBlock(), Block.UPDATE_ALL);
          level.scheduleTick(pos, fluid.getFluid(), 10 + level.getRandom().nextInt(30));
        }
      }
    }
  }
  ...
  public void spawnSmokeParticles(ServerLevel level) {
    ...
    level.sendParticles(
        ParticleTypes.SMOKE,
        pos.getX() + 0.5 + level.getRandom().nextGaussian() * 0.2,
        pos.getY() + 0.5 + level.getRandom().nextGaussian() * 0.2,
        pos.getZ() + 0.5 + level.getRandom().nextGaussian() * 0.2,
        1,
        0,
        0.05,
        0,
  ...
  public void loadAdditional(CompoundTag tag, HolderLookup.Provider provider) {
    ...
    tag.getFloat("Temperature").ifPresent(val -> internalTemperature = val);
  }
}
